...
| `coach/phase-artifacts/` | 教练 | Phase 1/3/4 的教练评价、补漏、标准框架 |
| `coach/feedback/` | 教练 | Phase 2 的逐题反馈、判断、修正推理路径 |
...
3. 教练生成 `coach/phase-artifacts/phase{N}.md`。
4. 使用 `save-phase-artifact` 登记教练产物。脚本会再次检查学生尝试是否非空。
5. 使用 `reveal-phase` 把教练产物显示给学生。

在步骤 2 之前调用 `save-phase-artifact` 会失败；在步骤 4 之前调用 `reveal-phase` 会失败。

### Phase 2

1. 教练把题目写入 `shared/questions/{id}.md`，并用 `start-question` 登记。
2. 学生作答到 `student/answers/{id}.md`，并用 `submit` 登记。
3. 教练生成 `coach/feedback/{id}.md`，并用 `save-feedback` 登记。
4. 使用 `reveal-feedback` 显示反馈。

在步骤 2 之前调用 `save-feedback` 或 `reveal-feedback` 会失败。
